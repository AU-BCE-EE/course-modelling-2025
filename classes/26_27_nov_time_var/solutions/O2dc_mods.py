...
Description:
    ...
    O2 is oxygen of course. This version includes an O2dcv() function 
    with time-variable rate constant k.
"""
...
def O2dcv(
    ...
    Dynamic model for dissolved oxygen diffusion and BOD degradation 
    in a stagnant mixture. BOD degradation modeled as a second-order
    reaction. This is a node-based version with time-variable reaction
    rate.
    ...
    dx: float
        Distance between nodes (m)
    k_dat : DataFrame
        Pandas DataFrame with second-order rate constant for BOD 
        oxidation (m3/kg-s) in column `k` and time (s) in column 
        `time`
    k_kind: str,
        Interpolation kind for interp1d() function for k, either 
        "linear" or "cubic"
    tmax : float
        Duration of simulation (s)
    nt : int
    ...
    # Create k function
    k_func = interp1d(k_dat["time"], k_dat["k"], kind = k_kind)
    ...
    def rates(
        ...
        O2_sat : float
            O2_sat from main function
        k_func : function
            Interpolation function for rate constant k
        D : float
            D from main function
    ...
    sol = solve_ivp(
        ...
        method = "LSODA",
        t_eval = np.linspace(0, tmax, nt), 
        args = (dx, x_grid, O2_sat, k_func, D))
    ...
    # Get k for output for checking
    k = k_func(np.linspace(0, tmax, nt))
