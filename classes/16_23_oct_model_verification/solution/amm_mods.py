...
Description:
    ...
    **errors** because it is part of an exercise on verification!
...
def dynmodu(rt,
    ...
    c_bg : float
        Concentration of NH3 (g) in ambient (background) air 
        (kg/m3 as N).
    pH : float
        Manure pH (pH units).
    ku : float
    ...
    def rates(t, conc):
        ...
        dc_urea_dt = urea_in - urea_hyd - urea_out 
...
def dynmodc(rt,
    ...
    Dynamic model for ammonia volatilization from a manure storage 
    tank with constant and equal manure flow in and out and a constant
    volume of manure. Includes urea hydrolysis and returns cumulative 
    volatilization and addition to and from tank.
    ...
    def rates(t, conc):
        ...
        return np.array([dc_TAN_dt, dc_urea_dt, TAN_in + urea_in, TAN_out + urea_out, volat])

    # Now solve with solve_ivp()
    res = solve_ivp(rates, t_span = [min(times), max(times)], 
                    y0 = [c_TAN_0, c_urea_0, 0., 0., 0.], t_eval = times)
...
def ssmod(rt,
    ...
    bb = rt * kl * c_bg * hen / depth

    c_TAN = (c_TAN_in + bb) / (1 + kk)
